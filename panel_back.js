$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.success) {
                ...
                let allRowsHtml = ''; 
                // Initialize the dropdown options with the default "All"
                ...
                response.domains.forEach(function(d, index) {
                    ...
                    // Populate the DNS Dropdown for the Security Tab
                ...
                // 1. First, physically write the new domains into the DOM
                container.html(allRowsHtml);
                
                // 2. NOW, check the badges because they actually exist on the page
                if (typeof runAsyncDnsChecks === "function") {
                    runAsyncDnsChecks();
                }

                // 3. Inject the options into the Security Tab's DNS dropdown
                $('#dnsDomainSelector').html(dnsDropdownOptions);
                
                // 4. Populate the generic domain dropdowns used in other Modals (like Let's Encrypt)
 ...
 // === Admin Profile Password Change ===
    ...
    // ==========================================
    // AJAX: Submit Create Domain / Subdomain
    // ==========================================
    $('#addDomainForm').on('submit', function(e) {
        e.preventDefault();
        ...
        // serialize() automatically captures the 'is_subdomain=true' if the switch is active, 
        // as well as the dynamically visible inputs.
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Reset the toggle state back to default primary domain view
                    $('#isSubdomainToggle').prop('checked', false).trigger('change');
                    
                    showToast(response.message);
                    
                    // Optional: Trigger your dynamic domain list refresh here
                    // fetchDomains(); 
                } else {
                    alert("Error: " + response.error);
                }
            ...
            error: function() {
                alert("A critical network error occurred. Check browser console.");
            },
            complete: function() {
    ...
    // Fetch the versions immediately so the modal is ready
    fetchInstalledPhpVersions();
    // ==========================================
...
function fetchInstalledPhpVersions() {
        $.ajax({
            url: '/ajax/get_php_versions.php', // Pointing to your custom file
            type: 'POST',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    let select = $('#phpVersionSelect');
                    select.empty(); // Clear the "Scanning server..." text
                    
                    if (response.versions.length > 0) {
                        response.versions.forEach(function(ver, index) {
                            // Automatically select the highest/newest version
                            let isSelected = (index === 0) ? 'selected' : '';
                            select.append(`<option value="${ver}" ${isSelected}>PHP ${ver} (FPM)</option>`);
                        });
                    } else {
                        select.append('<option value="">No PHP engines found</option>');
                    }
                }
            },
            error: function() {
                $('#phpVersionSelect').html('<option value="">Error fetching engines</option>');
            }
        });
    }
// ==========================================
// 3. The Async DNS Checker
// ==========================================
// Call this function IMMEDIATELY after your JS finishes rendering the Domain Accordion
function runAsyncDnsChecks() {
            ...
            success: function(response) {
                ...
                if (response.success) {
                    if (response.pointing) {
                        badge.removeClass('bg-secondary text-dark')
                             .addClass('bg-success text-white')
                             .html('<i class="bi bi-check-circle"></i> Pointing');
                    } else {
                        // Include the IP it's currently pointing to as a tooltip
                        let titleText = response.resolved_ip ? `Pointing to: ${response.resolved_ip}` : 'No DNS Record Found';
                        badge.removeClass('bg-secondary text-dark')
                             .addClass('bg-danger text-white')
                             .attr('title', titleText)
                             .html('<i class="bi bi-x-circle"></i> Not Pointing');
                    }
                } else {
