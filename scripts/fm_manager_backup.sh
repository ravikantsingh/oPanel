...
# STACKRIUM HOTFIX: PHP 8 AJAX Payload Bug in TFM
sed -i 's/json_decode($input, true) : \$_POST;/json_decode($input, true) ?: \$_POST;/g' "$FM_DIR/index.php"
sed -i '/if (isset($_POST\['\''type'\''\]) && $_POST\['\''type'\''\]==\"backup\")/i \    if(isset($_POST["content"])) { $_POST["savedata"] = $_POST["content"]; }' "$FM_DIR/index.php"
