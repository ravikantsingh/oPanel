...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'login') {
    ...
    if ($admin && password_verify($password, $admin['password_hash'])) {
        ...
        $logMessage = "[" . date('Y-m-d H:i:s') . "] Stackrium Control Auth Failed: Invalid credentials for user '{$username}'. IP: {$ip}\n";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'verify_2fa') {
    ...
    if (TOTP::verifyCode($secretToCheck, $code)) {
        ...
        $ip = $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN';
        $logMessage = "[" . date('Y-m-d H:i:s') . "] Stackrium Control Auth Failed: Invalid 2FA code. IP: {$ip}\n";
        file_put_contents('/opt/panel/logs/auth.log', $logMessage, FILE_APPEND);
    }
}
...
<head>
    ...
    <title>Stackrium Control | Systems Access</title>
    ...
    <?php elseif ($step === 2): ?>
        <div class="text-center">
            ...
            <div class="bg-white p-2 rounded-3 d-inline-block mb-3">
                <img src="<?= TOTP::getQRCodeUrl('Stackrium Control_Admin', $_SESSION['temp_2fa_secret']) ?>" class="img-fluid" alt="QR Code" style="width: 150px;">
            </div>
            <form method="POST">
