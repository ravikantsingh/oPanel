    ...
    <div class="accordion shadow-sm border-0" id="manualAccordion">
        ...
        <div class="accordion-item border-0 border-bottom">
            <h2 class="accordion-header" id="headingWeb">
                <button class="accordion-button collapsed fw-bold bg-light text-dark" type="button" data-bs-toggle="collapse" data-bs-target="#collapseWeb">
                    <i class="bi bi-cpu-fill me-2 text-success"></i> 2. Domains, Subdomains & App Engines
                </button>
            </h2>
            <div id="collapseWeb" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">Subdomain Architecture & Security</h6>
                    <ul class="text-muted small mb-3">
                        <li class="mb-2"><strong>Isolated Environments:</strong> Subdomains (e.g., <code>api.example.com</code>) are not just aliases; they are provisioned with their own dedicated <code>public_html</code> directories and PHP-FPM pools for maximum security.</li>
                        <li class="mb-2"><strong>DNS Injection:</strong> When you create a subdomain, Stackrium automatically injects the new A-Records directly into the parent domain's BIND9 zone file and safely increments the DNS serial number.</li>
                        <li class="mb-2"><strong>Ownership Verification:</strong> To prevent abuse, the backend strictly verifies that the currently logged-in system user actually owns the parent domain before allowing the subdomain to be provisioned.</li>
                    </ul>

                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">PHP Hot-Swapping & Software Center</h6>
                    <ul class="text-muted small mb-3">
                        <li class="mb-2"><strong>Live Version Changes:</strong> Need to upgrade a domain from PHP 8.1 to 8.3? Click "Change PHP". Stackrium safely rewrites the Nginx FastCGI socket path and reloads the worker pool with zero downtime.</li>
                        <li class="mb-2"><strong>Legacy Support:</strong> If you need older engines (like PHP 7.4) for legacy apps, visit the <strong>Software Center</strong> to install them globally. Once installed, they become instantly available for per-domain assignment.</li>
                    </ul>

                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">Framework-Specific Deployments</h6>
                    <ul class="text-muted small mb-3">
                        <li class="mb-2"><strong>Laravel Environment:</strong> When you click "Deploy Laravel", Stackrium automatically reconfigures Nginx to point the document root to the <code>/public</code> directory. It also installs Composer dependencies.</li>
                    ...
                    <h6 class="fw-bold text-dark border-bottom pb-1">Git Repository Auto-Deployment</h6>
            ...
            <div id="collapseAdvanced" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <ul class="text-muted small mb-3">
                        ...
                        <li class="mb-2"><strong>MIME Types:</strong> Nginx needs a dictionary to understand file types. If your users are trying to view an image or app file but it forces a "Download" instead, you need to add a MIME type.</li>
                    ...
                    <ul class="text-muted small mb-0">
                        <li><strong>Guardrails:</strong> Stackrium hard-caps Redis at <strong>128MB</strong> of RAM and uses an LRU (Least Recently Used) eviction policy to prevent Out-Of-Memory (OOM) crashes.</li>
                        <li><strong>WordPress:</strong> Use the 1-Click "Enable Redis" button to automatically install the Redis Object Cache plugin.</li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <div id="collapseDb" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <ul class="text-muted small">
                        ...
                        <li><strong>Databases:</strong> When provisioning a database, use <strong>Access Control</strong> to define security. Select <em>Localhost Only</em> for maximum security, or input a specific IP if connecting remotely.</li>
        ...
        <div class="accordion-item border-0 border-bottom">
            ...
            <div id="collapseSecurity" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">ModSecurity WAF & Custom Rules</h6>
                    <ul class="text-muted small mb-3">
                        <li class="mb-2"><strong>Per-Domain Isolation:</strong> Toggle the Web Application Firewall on/off per domain. This protects against SQL injections, XSS, and malicious bots by inspecting packets against the OWASP Core Rule Set.</li>
                        <li class="mb-2"><strong>Custom Rules with Dry-Run:</strong> You can write custom ModSecurity rules directly from the dashboard to whitelist IPs or block specific payloads. <em>Safety mechanism:</em> Stackrium performs a strict <code>nginx -t</code> syntax dry-run in the background. If your custom rule contains a typo, the system instantly aborts the save and rolls back to prevent your server from crashing!</li>
                    </ul>

                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">DNS & SSL</h6>
                    <ul class="text-muted small mb-3">
        ...
        <div class="accordion-item border-0 border-bottom">
            <h2 class="accordion-header" id="headingUpdates">
                <button class="accordion-button collapsed fw-bold bg-light text-dark" type="button" data-bs-toggle="collapse" data-bs-target="#collapseUpdates">
                    <i class="bi bi-cloud-arrow-down-fill me-2 text-primary"></i> 10. System Updates & Migrations
                </button>
            </h2>
            <div id="collapseUpdates" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    <p class="text-muted mb-3">Stackrium is designed to be fully self-healing with an Enterprise Auto-Update Engine.</p>
                    <ul class="text-muted small mb-0">
                        <li class="mb-2"><strong>Autonomous Nightly Updates:</strong> Every day at 4:00 AM, the server pings Stackrium Central. To prevent global server crashes, updates use a <strong>Staggered Rollout</strong> algorithm. Your server might safely wait 1 to 3 days after a release before autonomously applying the update in the background.</li>
                        <li class="mb-2"><strong>Safe Database Migrations:</strong> When an update requires a schema change, Stackrium automatically runs the pending SQL migrations during the Rsync process.</li>
                        <li class="mb-2"><strong>Manual Overrides:</strong> Go to the <strong>License & Updates</strong> tab and click "Fetch Updates" to bypass the stagger queue, forcefully execute the bash updater, and apply the release immediately.</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="accordion-item border-0 border-bottom">
            <h2 class="accordion-header" id="headingLicense">
                <button class="accordion-button collapsed fw-bold bg-light text-dark" type="button" data-bs-toggle="collapse" data-bs-target="#collapseLicense">
                    <i class="bi bi-credit-card-fill me-2 text-success"></i> 11. Commercial Licensing & Billing
                </button>
            </h2>
            <div id="collapseLicense" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    <p class="text-muted mb-3">Ah, the dreaded "Commercial License" section. Our billing department has rigorously audited your account and concluded that you currently owe us exactly <strong>$0.00</strong>.</p>
                    <p class="text-muted mb-0">Stackrium is 100% free to use. The <code>license.key</code> generated during your server's installation is simply a cryptographic handshake. It allows your server to securely authenticate with Stackrium Central to pull down core updates and manage staggered rollouts without getting rate-limited. Your "license" is automatically valid for the next 10 years (or until the heat death of the universe, whichever comes first). Keep your wallet in your pocket and enjoy building!</p>
                </div>
            </div>
        </div>
        ...
        <div class="accordion-item border-0">
            <h2 class="accordion-header" id="headingFaq">
                <button class="accordion-button collapsed fw-bold bg-dark text-white" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFaq">
                    <i class="bi bi-question-circle-fill me-2 text-warning"></i> 12. Frequently Asked Questions (FAQ)
                </button>
            </h2>
