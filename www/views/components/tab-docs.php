...
<div class="tab-pane fade" id="docs" role="tabpanel">
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
        <h4 class="mb-0 fw-bold text-dark"><i class="bi bi-book text-primary me-2"></i> Stackrium Control Official User Manual</h4>
    ...
    <div class="alert alert-warning shadow-sm border-warning border-start-0 border-end-0 border-bottom-0 border-3 rounded-0 mb-4 pb-3 pt-3">
        ...
        <p class="small mb-0 text-dark">Stackrium Control strictly manages your server's internal firewall (UFW). However, if you are hosting on AWS, Google Cloud, DigitalOcean, or Azure, you <strong>MUST</strong> also open the following ports in your provider's external Security Group / Network Firewall:</p>
        <ul class="small mb-0 mt-2 text-dark font-monospace">
            <li><strong>TCP 80 & 443:</strong> Web Traffic (HTTP/HTTPS)</li>
            <li><strong>TCP 7443:</strong> Stackrium Control Dashboard Access</li>
    ...
    <div class="accordion shadow-sm border-0" id="manualAccordion">
        ...
        <div class="accordion-item border-0 border-bottom">
            ...
            <div id="collapseWorkflow" class="accordion-collapse collapse show" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    <p class="text-muted mb-3">Stackrium Control utilizes a strict permission and isolation architecture. To successfully get a website or app online, you must follow this exact order of operations:</p>
        ...
        <div class="accordion-item border-0 border-bottom">
            ...
            <div id="collapseWeb" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    <p>The <strong>Web & Git</strong> tab is the core engine for hosting applications. Stackrium Control automatically configures Nginx, FastCGI Process Managers, and PM2 background workers behind the scenes.</p>
                    
                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">Framework-Specific Deployments</h6>
                    <ul class="text-muted small mb-3">
                        <li class="mb-2"><strong>Laravel Environment:</strong> When you click "Deploy Laravel", Stackrium Control automatically reconfigures Nginx to point the document root to the <code>/public</code> directory. It also installs Composer dependencies. <em>Note: Laravel 11 requires SQLite. Ensure <code>php8.3-sqlite3</code> and <code>php8.3-xml</code> are installed via the Software Center.</em></li>
                        <li class="mb-2"><strong>Python (WSGI/ASGI):</strong> Python apps run in isolated virtual environments (<code>venv</code>). Stackrium Control uses PM2 to keep the process running forever. Nginx is reconfigured as a <strong>Reverse Proxy</strong>, securely forwarding external port 80/443 traffic to your internal Python app.</li>
                    ...
                    <ul class="text-muted small mb-3">
                        ...
                        <li><strong>Safety Net:</strong> If your folder is empty after reverting, Stackrium Control automatically injects a default <code>index.html</code> template so your website doesn't show a blank "Invalid Response" error.</li>
                    </ul>

                    <h6 class="fw-bold mt-3 text-dark border-bottom pb-1">Git Repository Auto-Deployment</h6>
                    <ul class="text-muted small mb-4">
                        <li>Stackrium Control enforces a <strong class="text-dark">"One User, One Identity"</strong> rule. Each system user gets one unique SSH Deploy Key. If deploying different private repos to different domains, create a new System User for each project.</li>
                        <li><strong>Webhooks:</strong> Once cloned, Stackrium Control provides a unique Webhook URL. Add this to your GitHub/GitLab repository settings (Content type: <code>application/json</code>). Every code push will automatically trigger Stackrium Control to pull the latest changes!</li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <div id="collapseAdvanced" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <ul class="text-muted small mb-0">
                        <li><strong>Guardrails:</strong> Stackrium Control hard-caps Redis at <strong>128MB</strong> of RAM and uses an LRU (Least Recently Used) eviction policy. This prevents Redis from causing an Out-Of-Memory (OOM) server crash.</li>
            ...
            <div id="collapseDb" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <ul class="text-muted small">
                        ...
                        <li><strong>phpMyAdmin:</strong> Click the green <i class="bi bi-database-fill-gear"></i> icon next to any database to securely launch phpMyAdmin. Stackrium Control uses SSO, meaning you never have to type your database password.</li>
            ...
            <div id="collapseSecurity" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <ul class="text-muted small">
                        ...
                        <li class="mb-2"><strong>Initialize New Zone:</strong> If you want Stackrium Control to act as your Master DNS server, click this to generate the baseline BIND9 records (A, MX, TXT) automatically. <em>(Note: You must set ns1/ns2 at your registrar).</em></li>
        ...
        <div class="accordion-item border-0 border-bottom">
            ...
            <div id="collapseBackups" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    <p class="text-muted mb-3">Never lose your data. Stackrium Control handles automated compression and SQL dumping via Python daemon workers.</p>
                    <ul class="text-muted small">
                        ...
                        <li><strong>Auto-Schedule:</strong> Set up Daily, Weekly, or Monthly automated backups. Set a "Retention Limit" (e.g., 3 days) and Stackrium Control will automatically delete older backups to prevent your disk from filling up.</li>
        ...
        <div class="accordion-item border-0 border-bottom">
            ...
            <div id="collapseCron" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    ...
                    <ul class="text-muted small">
                        <li><strong>Add Cron Job:</strong> Schedule commands to run as specific system users. Stackrium Control uses standard cron syntax <code>* * * * *</code> (Minute, Hour, Day, Month, Weekday).</li>
        ...
        <div class="accordion-item border-0 border-bottom">
            ...
            <div id="collapseMail" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-white">
                    <p class="text-muted mb-3">Stackrium Control uses a Modular Mail Architecture. By default, the mail engine is completely uninstalled to save your server's RAM and CPU.</p>
                    <ul class="text-muted small mb-0">
        ...
        <div class="accordion-item border-0">
            ...
            <div id="collapseFaq" class="accordion-collapse collapse" data-bs-parent="#manualAccordion">
                <div class="accordion-body text-sm bg-light">
                    ...
                    <p class="text-muted mb-3 pb-2 border-bottom"><strong>A:</strong> This is almost always a DNS or Firewall issue. First, verify in your domain registrar (e.g., GoDaddy) that the A-Record points to your Stackrium Control server IP. Second, ensure Port 80 and Port 443 are open in your cloud provider's network security group.</p>
                    ...
                    <p class="text-muted mb-3 pb-2 border-bottom"><strong>A:</strong> Whenever you change advanced PHP settings (like <code>memory_limit</code>), Stackrium Control automatically tests syntax and restarts the PHP-FPM worker for that domain. If it didn't reflect, check the <strong>Live Task Log</strong>. If you entered invalid syntax, the server will block the reload to keep your site online.</p>
                    ...
                    <p class="text-muted mb-0"><strong>A:</strong> You will need SSH access to the server. Log in to your terminal and simply run the command: <code>sudo stackrium login</code>. The Stackrium Control CLI will instantly generate a secure, one-time access link. Copy and paste that link into your browser to bypass the login screen. Once inside, go to <strong>System Settings</strong> to reset or disable your 2FA.</p>
